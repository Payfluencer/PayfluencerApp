    ...
    if (err && err.response) {
      ...
      Logger.error({
        message:
          ...
          (err && err.message ? err.message : err?.toString?.() || String(err)),
      });
    }
...
export const googleAuthCallback = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const userInfoResult = await getGoogleUserInfo(access_token);

    if (userInfoResult.status === "error" || !userInfoResult.data) {
      return res.status(HttpStatusCode.BadRequest).json({
        status: "error",
        message: userInfoResult.message || "Failed to verify Google user info",
        data: null,
      });
    }
